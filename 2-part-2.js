    ...
    gameSubsets.forEach(subset => {
        ...
        cubes.forEach(cube => {
            ...
            if (cube.match(/\d blue/g)) {
                ...
                if (blueQuantity > maxBlue) { maxBlue = blueQuantity }
            }

            if (cube.match(/\d green/g)) {
                let greenQuantity = +cube.replace(/\sgreen/g,'');

                if (greenQuantity > maxGreen) { maxGreen = greenQuantity }
            }

            if (cube.match(/\d red/g)) {
                let redQuantity = +cube.replace(/\sred/g,'');

                if (redQuantity > maxRed) { maxRed = redQuantity }
            }
        })
    })
    console.log(JSON.stringify("maxBlue: " + maxBlue + " | maxGreen: " + maxGreen + " | maxRed: " + maxRed))
